package com.zwiki.memory.config;

import com.alibaba.cloud.ai.autoconfigure.memory.ChatMemoryAutoConfiguration;
import com.zwiki.memory.mem0.MemZeroMemoryStore;
...
public class MemZeroChatMemoryAutoConfiguration {
    ...
    public MemZeroServiceClient elasticsearchRestClient(MemZeroChatMemoryProperties properties, ResourceLoader resourceLoader) {
        ...
        // 如果配置了API Key，使用Mem0 Cloud API，无需调用configure
        if (properties.getClient().getApiKey() != null && !properties.getClient().getApiKey().isEmpty()) {
            logger.info("Using Mem0 Cloud API with API Key authentication.");
        } else {
            // 将client配置项交给Server初始化Mem0实例（自托管模式）
