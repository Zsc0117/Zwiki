  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.code === 200) {
        message.success(sourceType === 'zip' ? '任务已创建，文件正在后台解压处理中' : '添加仓库成功');
        form.resetFields();
        setFileList([]);
      ...
      if (error.errorFields) {
        ...
        message.error(error?.normalized?.message || '添加仓库失败');
      }
    } finally {
      setLoading(false);
    }
          ...
          {sourceType === 'git' && (
            <>
              ...
              <Form.Item
                ...
                autoComplete="off"
              >
                <Input placeholder="请输入Git仓库用户名（如需要）" autoComplete="off" />
              </Form.Item>
              ...
              <Form.Item
                ...
                label="密码"
              >
                <Input.Password placeholder="请输入Git仓库密码（如需要）" autoComplete="new-password" />
              </Form.Item>
            </>
          )}
        </motion.div>
